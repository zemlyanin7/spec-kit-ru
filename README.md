...
- [🔧 Справочник по Specify CLI](#-справочник-по-specify-cli)
...
Чтобы обновить Specify:

```bash
uv tool install specify-cli --force --from git+https://github.com/zemlyanin7/spec-kit-ru.git
```
...
Вызовите команду **`/speckit.constitution`**, чтобы сформировать «конституцию» проекта — набор принципов и правил разработки:

```bash
/speckit.constitution Сформулируй принципы, ориентированные на качество кода, стандарты тестирования, единообразие пользовательского опыта и требования к производительности
```

### 3. Подготовьте спецификацию

Используйте **`/speckit.specify`**, чтобы описать, что вы хотите построить. Фокусируйтесь на **что** и **почему**, а не на технологиях.

```bash
/speckit.specify Создай приложение, которое поможет организовывать фотографии в отдельные фотоальбомы. Альбомы группируются по дате и могут переупорядочиваться перетаскиванием на главной странице. Альбомы не вложенные. Внутри альбома фотографии отображаются плиткой.
```

### 4. Постройте технический план

Команда **`/speckit.plan`** фиксирует выбранный технологический стек и архитектуру.

```bash
/speckit.plan Приложение использует Vite с минимальным числом библиотек. По возможности — чистые HTML, CSS, JavaScript. Изображения никуда не загружаются, метаданные хранятся в локальной базе SQLite.
```

### 5. Разбейте задачи

**`/speckit.tasks`** сформирует список практических задач на основе плана:
...
**`/speckit.implement`** выполнит задачи и соберёт функциональность в соответствии с планом:
...
## 🔧 Справочник по Specify CLI

### Основные команды

| Команда | Описание |
|---------|----------|
| `specify init <name>` | Инициализация проекта |
| `specify init --ai <agent>` | Инициализация с выбранным ИИ |
| `specify init --script {sh|ps}` | Выбор типа скриптов |
| `specify check` | Проверка окружения и подготовка |
| `/speckit.constitution` | Генерация «конституции» проекта |
| `/speckit.specify` | Создание спецификации |
| `/speckit.plan` | План реализации |
| `/speckit.tasks` | Разбиение на задачи |
| `/speckit.implement` | Исполнение задач |
...
### Шаг 1. Создать конституцию `/speckit.constitution`

Определяем фундаментальные принципы: цели качества, стандарты тестирования, UX, производительность.

### Шаг 2. Сформулировать спецификацию `/speckit.specify`
...
### Шаг 4. Составить план `/speckit.plan`
